// Interview Mode userscript for claude.ai. See interview_chatgpt.js for the
// shared bridge protocol.
(function () {
  'use strict';
  if (window.location.host.indexOf('claude.ai') === -1) return;

  const BRIDGE = window.__INTERVIEW_BRIDGE__ || {};
  const MODE = BRIDGE.mode || 'suggest';

  const SELECTORS = {
    composer: 'div[contenteditable="true"][data-placeholder], div[contenteditable="true"]',
    sendButton: 'button[aria-label*="Send" i], button[data-testid*="send" i]',
    assistantTurn: '[data-testid*="assistant" i], [data-is-streaming], div.font-claude-message',
    loginIndicator: 'a[href*="login"], button[data-testid*="login" i]',
    userIndicator: '[data-testid*="user-menu" i], [data-testid*="profile" i]',
  };
  ...
  async function isLoggedIn() {
    if (document.querySelector(SELECTORS.userIndicator)) return true;
    if (document.querySelector(SELECTORS.loginIndicator)) return false;
    try {
  ...
  async function run() {
    try {
      ...
      post('login-status', { signed_in: loggedIn ? 'true' : 'false', provider: 'claude' });
      ...
      post('result', { data: text, provider: 'claude' });
    } catch (err) {
